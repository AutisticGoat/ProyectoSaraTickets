...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 01-12-2024 a las 18:57:59
...
(3, 'Problema con teléfono de empresa: Parte de la pantalla táctil no responde', 'Solucionado'),
(4, 'Problema con computadora: Teclado no responde de forma apropiada (no funcionan las teclas I, O ni K)', 'Solucionado'),
...
--
DELIMITER $$
CREATE TRIGGER `actualizar_horasalida` AFTER UPDATE ON `problema` FOR EACH ROW BEGIN
    IF NEW.Estatus = 'Solucionado' THEN
        UPDATE ticket
        SET HoraSalida = NOW()
        WHERE ID = (SELECT IdTicket FROM problematicket WHERE IdProblema = NEW.ID );
    END IF;
END
$$
DELIMITER ;
...
(5, '2024-11-08 20:52:30', '2024-12-01 10:56:54'),
(6, '2024-11-08 21:02:10', '2024-12-01 10:57:03'),
